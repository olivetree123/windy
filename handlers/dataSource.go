...
func DataSourceDBCreateHandler(c *coco.Coco) coco.Result {
	...
	if dataSource, _ := models.GetDataSourceDB(param.Name); dataSource != nil {
		log.Error("dataSource exists, name=", param.Name)
		return coco.ErrorResponse(100)
	}
...
func DataSourceTableCreateHandler(c *coco.Coco) coco.Result {
	...
	if table, _ := models.GetDataSourceTable(param.DbID, param.Name); table != nil {
		log.Error("table already exists")
		return coco.ErrorResponse(100)
	}
	table, err := models.NewDataSourceTable(param.DbID, param.Name, param.Fields)
	if err != nil {
		log.Error(err)
		return coco.ErrorResponse(100)
	}
